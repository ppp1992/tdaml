...
def test_projections_1(tol=1e-5):
    nn = 3
    rho = 1.0
    w1 = np.array([1, 1, 1])
    w1 = w1 - w1.mean()
...
'''
david's code for solving max pz
'''


def solve_argmax_p_for_pz(z, rho, tol=1e-10):
    assert (rho > 0)
    rho = float(rho)
    ...
    lam_min = 0.0
    # lam_max = (1 / nn) * (nn * w_sort[0] / np.sqrt(2. * rho + 1.) - 1.)
    lam_max = max(nn * w_sort[0], np.sqrt(nn / (2 * rho)) * np.linalg.norm(w_sort))
    lam_init_max = lam_max

    if (lam_max <= 0):  # optimal lambda is 0
        (eta, ind) = find_shift(w_sort, w_sort_cumsum, nn, 0., rho)
        p = z - eta
        low_inds = p < 0
        p[low_inds] = 0.
        return p
    ...
    while (lam_max - lam_min > tol * lam_init_max):
        lam = .5 * (lam_max + lam_min)
        (eta, ind) = find_shift(w_sort, w_sort_cumsum, nn, lam, rho)
        dd = 1.0 / (2 * lam ** 2) * w_sort_sqr_cumsum[ind] + eta ** 2 * ind ** 2 / 2 + eta * w_sort_cumsum[
            ind] / lam + (nn - ind) / (2 * nn ** 2) - rho / nn
        if (dd):
            # constraint infeasible, increase lam (dual var)
            lam_min = lam
        else:
...
def find_shift(w_sort, w_sort_cumsum, nn, lam, rho):
    i_low = 0
    i_high = int(nn)
    i = 0
    ...
    while i_low != i_high:
        i = (i_low + i_high) // 2
        s_left = (i * w_sort[i] - w_sort_cumsum[i]) / lam
        if i + 1 >= nn:
            eta = 1 / nn - 1 / i - w_sort_cumsum[i] / (lam * i)
            return eta, i
        s_right = ((i + 1) * w_sort[i + 1] - w_sort_cumsum[i + 1]) / lam
        if s_right >= 1 and s_left < 1:
            eta = 1 / nn - 1 / i - w_sort_cumsum[i] / (lam * i)
            return eta, i
        elif s_left >= 1:
            i_high = i
...
def test_solve_max():
    loss_rev = np.array([-1, -1.1, -1, -1.2])
    p = solve_argmax_p_for_pz(loss_rev, rho=1.01)
    print(p)


if __name__ == '__main__':
    w = np.array([-1, -1.1, -1.11])
    ...
    eta, i = solve_inner_eta(w_sort, w_sort_cumsum, nn, lam, rho)
    p = w - eta
    low_inds = p < 0
    p[low_inds] = 0
