...
def meta_train(model, n_epoch=3):
    ...
    paths = ['data/train_oneshot_a.pkl',
             ...
             'data/train_oneshot_d.pkl']

    optimizer = torch.optim.Adam(params=model.generator.parameters(), lr=1e-3, weight_decay=1e-5)
    task_size = 20
    batch_n_ID = 25
    ...
    for epoch_i_ in range(n_epoch):
        ...
        if epoch_i == 0:
            ...
            loader_b = fs_loaders[3]
        elif epoch_i == 2:
            loader_a = fs_loaders[1]
            loader_b = fs_loaders[2]
        else:
            loader_a = fs_loaders[3]
