...
def build_param_list():
    xs = np.array([0.0001, 0.0002, 0.0004, 0.0016, 0.0032])
...
def best_auc_func(params):
    ...
    if auc > best_auc:
        ...
        best_loss = loss
        best_params = params
    print('best--auc:{}, loss:{}'.format(best_auc, best_loss))
...
if __name__ == '__main__':
    ...
    conf.cvr_flag = 0
    i1, i2 = 0, 1
    save_path = 'models/models.txt'
    conf.model_type = 'DeepFM'
    max_evals = 10
    setup_seed(123)
    # setup_seed(123)
    ...
    # xs, ys = build_param_list()
    # hyper_param_analysis(xs, ys, save_path)
